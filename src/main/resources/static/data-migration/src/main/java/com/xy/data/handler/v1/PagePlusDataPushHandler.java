 ...
 **/
@Slf4j
public abstract class PagePlusDataPushHandler<T, R> extends DataPushHandler<T, R> {
    public PagePlusDataPushHandler(Class<T> tClass, Class<R> rClass, Class<?> mapper, String start, String end) {
    ...
    protected final void pushData(String msg, LocalDateTime start, LocalDateTime end) throws Exception {
        ...
        int total = total(start == null ? LocalDateTime.of(1, 1, 1, 0, 0) : start, end);

        log.info("开始迁移数据: " + msg + ",  分页总数量: {}", total);

        for (int i = 0; i < total; i = i + limit) {
            LocalDateTime finalStart = start == null ? LocalDateTime.of(1, 1, 1, 0, 0) : start;
